 ...
 /* private JPanel getPanelInicio(){
  ...
  private JPanel getPanelTrabajadores(){
    if(panelTrabajadores == null){
        if(panelClientes == null){
          panelClientes = new HandlerTrabajadores();
        }
    }
   ...
   // private JList listaRealizados;
    ...
    /* listaBuscados = new JList();
     ...
     /* activos = new JButton("Ver Activos");
      ...
      numL = new JLabel("Numero trabajador");
      this.add(numL);
      numL.setSize(75,25);
      numL.setLocation(500,225);
      
      gananciaL = new JLabel("Ganancia");
      this.add(gananciaL);
      gananciaL.setSize(75,25);
      gananciaL.setLocation(500,275);
      
      paswoordL = new JLabel("Paswoord");
      this.add(paswoordL);
      paswoordL.setSize(75,25);
      paswoordL.setLocation(500,325);
...
>>>>>>> .r28
      ...
      if(evento.getSource().getClass().getName().equals("javax.swing.JButton")){
        ...
        if((evento.getSource() == agregar) || (evento.getSource() == modificar) ){
          ...
          if(nombreP.length() > 0 && apellidoP.length() > 0){
            ...
            try{
              int cedulaP = Integer.parseInt(this.ci.getText());
              int numeroTrabajadorP = Integer.parseInt(this.numeroTrabajador.getText());
              int gananciasP = Integer.parseInt(this.ganancias.getText());
              char[] paswoordP = nombre.getText().toCharArray();
              
              if(evento.getSource() == agregar){
                Cliente tra = new Trabajador(nombreP, apellidoP, cedulaP,numeroTrabajadorP,gananciaP,paswoordP);
                
                if(!sistema.getEmpresa().agregarTrabajador(tra)){
                ...
                if (!listaTrabajadores.isSelectionEmpty()){
                  Trabajador tra = (Trabajador)listaTrabajadores.getSelectedValue();
                  tra.setNombre(nombreP);
                  tra.setApellido(apellidoP);
                  tra.setCedula(cedulaP);
                  tra.setNumTrabajador(numeroTrabajadorP);
                  tra.setGanancias(gananciasP);
                  tra.setPaswoord(paswoordP);
                }
                else{
          ...
          if (!listaClientes.isSelectionEmpty()){
            ...
            if (respuesta == JOptionPane.YES_OPTION){
              Trabajador tra = (Trabajador)listaTrabajadores.getSelectedValue();
              sistema.getEmpresa().eliminarTrabajador(tra);
            }
          }else{
            JOptionPane.showMessageDialog(null, "No hay cliente seleccionado" , "Atención", JOptionPane.INFORMATION_MESSAGE);
          }
   ...
   /* private void cambiarEstadoBotones(boolean estaHabilitado){
    ...
    public void valueChanged(ListSelectionEvent evento) {
      if (!listaTrabajadores.isSelectionEmpty()){
        Trabajador cli = (Trabajador)listaTrabajadores.getSelectedValue();
        nombre.setText(tra.getNombre());
        apellido.setText(tra.getApellido());
        ci.setText(""+tra.getCedula());
        cargarModelo(modeloRealizados, tra.getViajesRealizados());
        cargarModelo(modeloBuscados, cli.getDestinosBuscados());
      }
